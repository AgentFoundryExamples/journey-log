...
def character_wounded_with_quest(base_player_state):
    ...
    quest = Quest(
        ...
        rewards=QuestRewards(
            items=["Dragon Slayer Sword"], currency={"gold": 500}, experience=1000
        ),
...
def character_dead_completed_quest(base_player_state):
    ...
    quest = Quest(
        ...
        requirements=["Visit Ancient Ruins", "Find the artifact"],
        rewards=QuestRewards(items=["Ancient Artifact"], currency={}, experience=500),
    ...
    return CharacterDocument(
        ...
        additional_metadata={
            ...
            "death_timestamp": "2026-01-11T18:00:00Z",
        },
    )
...
def narrative_turn_different_timezone():
    ...
    return NarrativeTurn(
        ...
        timestamp=datetime(
            2026, 1, 12, 11, 0, 0, tzinfo=tz_offset
        ),  # 11am +5 = 6am UTC
    )
...
class TestCharacterDocumentRoundTrip:
    ...
    def test_healthy_character_roundtrip(self, character_healthy):
        ...
        assert (
            restored.narrative_turns_reference
            == character_healthy.narrative_turns_reference
        )
        ...
        assert (
            restored.player_state.identity.name
            == character_healthy.player_state.identity.name
        )
        assert (
            restored.player_state.identity.race
            == character_healthy.player_state.identity.race
        )
        assert (
            restored.player_state.identity.character_class
            == character_healthy.player_state.identity.character_class
        )
        ...
        assert len(restored.player_state.equipment) == len(
            character_healthy.player_state.equipment
        )
        ...
        assert len(restored.player_state.inventory) == len(
            character_healthy.player_state.inventory
        )
        ...
        assert (
            restored.player_state.additional_fields
            == character_healthy.player_state.additional_fields
        )
    ...
    def test_wounded_character_with_quest_roundtrip(self, character_wounded_with_quest):
        ...
        assert (
            restored.active_quest.name == character_wounded_with_quest.active_quest.name
        )
        ...
        assert (
            restored.active_quest.updated_at
            == character_wounded_with_quest.active_quest.updated_at
        )

    def test_character_in_combat_multiple_enemies_roundtrip(
        self, character_in_combat_multiple_enemies
    ):
        ...
        assert (
            restored.combat_state.combat_id
            == character_in_combat_multiple_enemies.combat_state.combat_id
        )
        ...
        assert (
            restored.combat_state.started_at
            == character_in_combat_multiple_enemies.combat_state.started_at
        )
    ...
    def test_dead_character_completed_quest_roundtrip(
        self, character_dead_completed_quest
    ):
...
class TestNarrativeTurnRoundTrip:
    ...
    def test_narrative_turn_no_turn_number_roundtrip(
        self, narrative_turn_no_turn_number
    ):
    ...
    def test_narrative_turn_timezone_conversion(
        self, narrative_turn_different_timezone
    ):
        """Test that non-UTC timezones are converted to UTC during round-trip."""
        # Original timestamp is 11am +5 = 6am UTC
        original_utc = narrative_turn_different_timezone.timestamp.astimezone(
            timezone.utc
        )
...
class TestPointOfInterestRoundTrip:
    ...
    def test_poi_discovered_not_visited_roundtrip(self, poi_discovered_not_visited):
        ...
        assert (
            restored.timestamp_discovered
            == poi_discovered_not_visited.timestamp_discovered
        )
        assert restored.visited is False
        assert restored.last_visited is None
        assert restored.notes == "Looks interesting, should explore later"

    def test_poi_visited_no_discovery_time_roundtrip(
        self, poi_visited_no_discovery_time
    ):
        ...
        assert (
            restored.timestamp_discovered is None
        )  # Edge case: visited but no discovery time
...
class TestRoundTripEdgeCases:
    ...
    def test_timestamp_timezone_aware_equality(self):
        ...
        offset_time = datetime(
            2026, 1, 11, 17, 0, 0, tzinfo=timezone(timedelta(hours=5))
        )
...
class TestNumericFieldSerializationExclusion:
    ...
    def test_multiple_entries_in_arrays(self, character_in_combat_multiple_enemies):
        ...
        assert len(restored.combat_state.enemies) == 3
        assert all(
            isinstance(enemy, EnemyState) for enemy in restored.combat_state.enemies
        )
